...
The command binds the Web UI to all interfaces by default and opens `http://127.0.0.1:3080` in the default browser for a local launch. Pass `--host 127.0.0.1` to restrict it to this machine. An SSH launch only prints the host URL because the SSH client or editor owns the local forwarded address. Pass `--no-open` to run the server without opening a browser. See [Web UI guide](docs/user/guide/index.md).
