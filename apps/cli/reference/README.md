...
The production Web runner needs built package and frontend artifacts (`pnpm run build`). It binds all interfaces by default, prints a discovered LAN URL, and, for a local launch, opens `http://127.0.0.1:3080` only after the complete Loader tree settles. Pass `--host 127.0.0.1` to restrict the server to this machine; use `--trusted-host` for a named authority. A non-empty inherited `SSH_CONNECTION` or `SSH_TTY` suppresses the browser handoff because the SSH client or editor owns the local forwarded address; the host URL is still printed. Immediately before a local handoff it prints `dsh web: opening the default browser; pass --no-open to disable`; if the operating-system handoff fails, a diagnostic on stderr states the reason, leaves the server running, and names the URL for manual use. The `/api` browser-trust fence is not authentication, so expose an all-interfaces deployment only to a trusted network.
